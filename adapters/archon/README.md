...
This adapter runs **team-rocket's process on that harness**. The split is clean:
...
- **team-rocket owns the behaviour at each node** — the discovery homework, the relentless plan interrogation of the lead, the quality bar, the review lens.
...
> This targets the **harness** on `main`. It is **not** the archived `archive/v1-task-management-rag` branch (MCP task-management + RAG) — that's a different product and not what this adapter wires.
