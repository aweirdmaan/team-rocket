...
├── discovery (prereq)         ← understand the problem + code before implementing
...
4. **Create the discovery prereq** as a child task. Description: "Understand the problem space and the code before defining goals. Read the relevant code and subsystems, map dependencies and integration points, identify constraints and risks, and (if the story comes with a test matrix or acceptance row list) map each row to either an existing test or a new one. No code." Acceptance: "Problem space and code documented. Constraints and risks captured. Goal candidates drafted with INVEST. Ready to define WHY + WHAT for each goal."

5. **Create 2-3 placeholder goal items** as children. Each with:
   - Title: "Goal N — define after discovery"
   - Description: "WHY: to be defined after discovery. WHAT: to be defined after discovery."
   - Acceptance: "To be defined. Use INVEST criteria: Independent, Negotiable, Valuable, Estimable, Small, Testable."

6. **Wire dependencies** so the goals are blocked by the discovery prereq (the lead's tracker should support this; if not, note it in the umbrella).

7. **Don't create implementation tasks yet** — those get filled in after discovery when the design is real.

8. **Tell the lead:**
   - The story has been scaffolded with a discovery prereq and placeholder goals.
...
- Don't skip the discovery prereq. The most common project failure is implementing before understanding.
