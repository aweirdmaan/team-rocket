...
2. If the input is an external tracker id (Jira etc.): fetch it using the access pattern the project documents (CLAUDE.md or TEAM-ROCKET.md). Mirror it into beads: `bd create --type=epic` with the WHY and WHAT. Note the external id in the epic description so `pr` and `retro` can sync back.
